...
const normalizePricingOptions = (
  ...
  if (rawPricing.length > 0) {
    return rawPricing.map((item, index) => ({
      tier:
        item.tier?.trim() ||
        `${locale === "es" ? "Opción" : "Option"} ${index + 1}`,
    ...
    ? [
        {
          tier: fallbackTier,
          price: fallbackPrice,
          amount: extractAmountFromPrice(fallbackPrice),
        },
      ]
    : [];
};
...
const normalizeService = (
  ...
  const detailImages = Array.isArray(rawService.images)
    ? rawService.images
        .map((image) =>
          normalizeImagePath(String(image.localPath ?? "").trim()),
        )
        .filter(Boolean)
    : [];
  const pricingOptions = normalizePricingOptions(rawService, category, locale);
  const fallbackImage = normalizeImagePath(
    String(rawService.image ?? "").trim(),
  );
  const image = detailImages[0] || fallbackImage || "/imgs/placeholder.jpg";
  const price =
    pricingOptions[0]?.price || String(rawService.price ?? "").trim();
  ...
  const transferRoutes = Array.isArray(rawService.transferRoutes)
    ? rawService.transferRoutes.map((route) => ({
        ...
        amount: Number.isFinite(Number(route.amount))
          ? Number(route.amount)
          : extractAmountFromPrice(String(route.price ?? "")),
...
const normalizeServices = (
  ...
  return services.map((service, index) =>
    normalizeService(service as RawService, index, category, locale),
  );
};
...
const fetchServices = async (
  ...
  try {
    ...
    return normalizeServices(
      (data as Record<string, unknown>)?.record ?? data,
      fallback,
      category,
      locale,
    );
...
const serializeToursForSave = (services: Tour[]): RawService[] =>
  services.map((service) => ({
    ...
    pricing: service.pricingOptions.map((option) => ({
      tier: option.tier,
      price: option.price,
    })),
...
export const saveTransportServices = async (
  services: Tour[],
  locale: Locale,
): Promise<void> => {
  await apiPut<unknown>(
    "transport-services",
    serializeTransportForSave(services),
    { locale },
  );
};
