...
function mergeConfigWithDefaults(
  partial: Partial<TransferConfig>,
): TransferConfig {
  const partialAny = partial as Partial<TransferConfig> & {
  ...
  return {
    ...
    vehicleTypes:
      Array.isArray(partial.vehicleTypes) && partial.vehicleTypes.length > 0
        ? partial.vehicleTypes.map((vt) => ({
            ...DEFAULT_TRANSFER_CONFIG.vehicleTypes.find(
              (dv) => dv.key === vt.key,
            ),
            ...vt,
          }))
    ...
    modifiers: {
      ...
      municipioMultipliers: {
        ...
        ...(partial.modifiers?.municipioMultipliers ??
          partialAny.municipioMultipliers ??
          {}),
      },
    },
  };
...
function ensureDistanceDiscountFields(config: TransferConfig): TransferConfig {
  if (
    config.modifiers.minimumPrice === undefined ||
    config.modifiers.minimumPrice === null
  ) {
    config.modifiers.minimumPrice =
      DEFAULT_TRANSFER_CONFIG.modifiers.minimumPrice;
  }

  if (config.modifiers.distanceDiscountMaxPercent === undefined) {
    config.modifiers.distanceDiscountMaxPercent =
      DEFAULT_TRANSFER_CONFIG.modifiers.distanceDiscountMaxPercent;
  }

  if (config.modifiers.distanceDiscountSaturationKm === undefined) {
    config.modifiers.distanceDiscountSaturationKm =
      (config.modifiers as any).distanceDiscountHalfKm ??
      DEFAULT_TRANSFER_CONFIG.modifiers.distanceDiscountSaturationKm;
  }
...
export async function createVehicleType(
  config: TransferConfig,
  newVt: TransferConfig["vehicleTypes"][0],
...
export async function updateVehicleType(
  config: TransferConfig,
  updatedVt: TransferConfig["vehicleTypes"][0],
): Promise<TransferConfig> {
  const updated = {
    ...config,
    vehicleTypes: config.vehicleTypes.map((v) =>
      v.key === updatedVt.key ? updatedVt : v,
    ),
  };
  await saveTransferConfig(updated);
  return updated;
...
export async function updateModifiers(
  config: TransferConfig,
  newModifiers: TransferConfig["modifiers"],
): Promise<TransferConfig> {
...
export async function updateMunicipioPriceMultipliers(
  config: TransferConfig,
